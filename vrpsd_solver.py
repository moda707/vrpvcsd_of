    ...
    def train(self):
        ...
        while not self.model.stop:
            ...
            while len(self.model.env.TT) > 0:
                ...
                served_demand = self.model.env.state_transition(k)
                prev_time = stime

                # select action
                x_k, selected_target_id, state, available_targets, is_terminal = \
                    self.model.choose_action(k, self.trials, True)

                reward_to_learn = self.model.compute_actual_reward(k, x_k)
                ...
                t_k = self.model.env.post_decision(x_k, k)
